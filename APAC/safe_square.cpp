...
long int safe( long int r,long int c, int** m, int** cont,long int size)
{
	...
	{	
		for( j = 0 ;j < c-size+1 ; j++ )
		{
			/*long int suc = 0;
			if(cont[i][j])
			//check if last matrix starting from i , j  of size size-1 was valid or not
			{
				for( k = 0; k < size ; k++ )
				{
					if(m[i][k])
					{
						cont[i][j] = 0;next = 1;
						cout<<"inner call"<<i<<" "<<j<<endl;print(cont,r,c);
						break;
					}
				}
				for( k = 0; k < size ; k++ )
			...
			//cout<<"start from ("<<i<<","<<j<<")"<<"for size "<<size<<endl;
			if(isSafe(m,i,j,size))
			{
				count++;
	...
	//cout<<"end of size: "<<endl;print(cont,r,c);
	//cout <<size<<" squares : "<<count<<endl;
	return count;
